...
inline const QList<SchedInfo> ALL_SCHEDULERS = {
    ...
    {"cake", "Cake", "Gaming / Low Latency",
     "CAKE DRR++ adapted for CPU scheduling. 4-tier classification (Critical/Interactive/Frame/Bulk), "
     "auto game detection, EEVDF-inspired weighting, yield-gated quantum, per-LLC DSQ sharding. "
     "Profiles: gaming, esports, legacy.",
     {"auto", "gaming", "lowlatency", "powersave", "server"}},

    {"chaos", "Chaos", "Testing / Debug",
     "Intentionally degrades performance to expose race conditions. Random delays, CPU freq scaling, "
    ...
    {"flow", "Flow", "Desktop / Deterministic",
     "Budget-driven scheduler with zero heuristics. Tasks accumulate budget while sleeping and spend it "
     "while running. 4 FIFO dispatch tiers rotate round-robin. No starvation. No modes — every decision "
     "comes from remaining budget.",
     {"auto"}},

    {"forge", "Forge", "Experimental / AI",
     "AI-optimised base scheduler evolved in place by scx_forge_agent. Default policy: wakeup placement, "
    ...
    {"mitosis", "Mitosis", "Cloud / HPC",
     "Cgroup-aware cell isolation. Each cgroup child gets a dedicated CPU set with shared dispatch queue. "
    ...
    {"pandemonium", "Pandemonium", "Desktop / Low Latency",
     "Behavioural-classification scheduler with topology-aware placement. 3-tier dispatch, damped harmonic "
     "oscillator CoDel stall detection, MWU adaptive control loop. Self-calibrates from 2C to 32C+.",
     {"auto", "gaming", "lowlatency", "powersave", "server"}},

    {"rlfifo", "RLFIFO", "Testing / Reference",
     "Simple FIFO round-robin based on scx_rustland_core. Educational template for building user-space "
